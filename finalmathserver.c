...
void* process_operations(void* arg) {
    ...
    for (int i = 0; i < ctx->op_count; i++) {
        ...
        if (strcmp(operation, "set") == 0) {
            ...
            int first = 1; // Flag to track the first prime number
            for (int j = 2; j <= ctx->value; j++) {
                if (is_prime(j)) {
                    char temp[16];
                    if (!first) {
                        // Append a comma and space before adding the next number
                        strncat(ctx->log_buffer[ctx->log_count - 1], ", ", 
                                MAX_BUFFER_SIZE - strlen(ctx->log_buffer[ctx->log_count - 1]) - 1);
                    }
                    snprintf(temp, sizeof(temp), "%d", j);
                    strncat(ctx->log_buffer[ctx->log_count - 1], temp, 
                            MAX_BUFFER_SIZE - strlen(ctx->log_buffer[ctx->log_count - 1]) - 1);
                    first = 0; // Set flag to false after the first number
                }
            }
            strncat(ctx->log_buffer[ctx->log_count - 1], ")\n", 
                    MAX_BUFFER_SIZE - strlen(ctx->log_buffer[ctx->log_count - 1]) - 1);
